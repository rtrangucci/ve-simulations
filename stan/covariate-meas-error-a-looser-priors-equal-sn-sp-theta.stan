...
transformed parameters {
  ...
  for (r in 1:N_r) {
    for (x in 1:N_x) {
      if (x == 1) {
        theta[r,x] = softmax(append_row(theta_raw[r],[0]'));
      } else {
        theta[r,x] = softmax(append_row(theta_raw[r] + beta_raw[x-1],[0]'));
      }
      for (z in 1:N_z) {
...
model {
  ...
  for (r in 1:N_r)
    theta_raw[r] ~ normal(prior_means_theta_raw, prior_sds_theta_raw);
  for (x in 1:(N_x-1)) {
...
generated quantities {
  ...
  for (z in 1:(N_z - 1)) {
    ...
    marg_ve_a_x[z] = dot_product(denom_ve, prop_x) - dot_product(num_ve, prop_x);
  }
}

...
parameters {
  array[N_r,N_x] vector[N_U - 1] theta_raw;
  array[N_U,N_x] vector[N_a - 1] p_a_raw;
  /* array[N_a] vector<lower=0, upper=1>[N_Y_1] p_y_1; */
  array[N_a,N_x] vector[N_Y_1] logit_p_y_1;
...
transformed parameters {
  ...
  for (a in 1:N_a)
    for (x in 1:N_x)
      p_y_1[a,x] = inv_logit(logit_p_y_1[a,x]);
  
  for (u in 1:N_U)
    for (x in 1:N_x)
      p_a[u,x] = softmax(append_row(p_a_raw[u,x],[0]'));
  for (r in 1:N_r) {
    for (x in 1:N_x) {
      theta[r,x] = softmax(append_row(theta_raw[r,x],[0]'));
      for (z in 1:N_z) {
...
model {
  ...
  for (u in 1:N_U)
    for (x in 1:N_x)
      p_a_raw[u,x] ~ normal(0, 1.7);
  for (r in 1:N_r)
    for (x in 1:N_x)
      theta_raw[r,x] ~ normal(prior_means, prior_sds);
  for (a in 1:N_a)
    for (x in 1:N_x)
      logit_p_y_1[a,x] ~ normal(-1, 1);
}
